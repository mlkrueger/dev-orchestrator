...
7. **Resource locks** — a `resource:<name>` label on any ticket that needs a shared mutable resource (e.g. `resource:db` for tickets whose tests reset a shared local database). Tickets sharing a resource label are serialized by default, so a lock is a parallelism tax — apply it only when contention is real, and prefer the two escapes: (a) if the project's harness can isolate instances (parameterized ports, per-run schemas/dirs), note it so the run can declare a `resource_pools` capacity in `.dev-orchestrator/config.json` and dispatch holders concurrently with distinct `RESOURCE_SLOT`s; (b) if many *small* tickets would share one lock — the classic case is a pile of 5-minute e2e-journey tickets all needing the same preview server — **batch related journeys into one ticket** (with per-journey criteria) instead of minting one ticket each, since a serialized lock makes per-ticket dispatch+gate overhead the dominant cost. Omit the label when no such resource exists.

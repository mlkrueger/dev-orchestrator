...
3. Load the `dev-orchestrator:tracker` skill (Skill tool). Read `.dev-orchestrator/config.json` if present.
   - **No tracker? Use a local build plan.** If `"tracker": "local"`, the ticket source is a build plan file in the repo (`.dev-orchestrator/build-plan.yaml` by default) — no external tracker, API key, or MCP server needed, and status is written back into the plan. Everything below works unchanged (`bin/tracker` routes to the file backend). If the user has no tracker configured and no plan, you can offer to scaffold one (see `docs/local-tracker.md`).

...
| `gate` | `ticket`, `gate` (`scope`\|`qa`\|`review`), `verdict` (`PASS`\|`FAIL`), `detail` | Every gate verdict |
...
| `event` | Fields |
|---|---|
| `agent_usage` | `ticket` (parsed from the `TICKET: <id>` first line of the dispatch prompt, else `null`), `agent`, `model`, `input_tokens`, `output_tokens`, `cache_creation_tokens`, `cache_read_tokens`, `turns`, `duration_s`, `source` (`sidechain`\|`main`), `session_id` |
...
The hook cannot see which ticket a subagent served — correlation relies on the dispatch prompt starting with `TICKET: <id>` on its own line. Orchestrators must honor this; gate agents dispatched for a ticket should carry the same first line so their cost rolls up to the ticket too.

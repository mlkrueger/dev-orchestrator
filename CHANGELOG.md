...
### Added
- **Local tracker — a build plan file as the ticket source** (`bin/tracker_local.py`, `{"tracker": "local"}`): for users with no external tracker (Linear/Jira/…) installed or wanted, a build plan file in the repo (`.dev-orchestrator/build-plan.yaml` by default; `.yml`/`.json` detected; override via `local.plan`) is the backlog, and ticket status is written back into it as work proceeds — so the plan doubles as the board and stays an accurate source of truth to resume from. `bin/tracker` now routes to the configured backend, so the CLI and every caller (orchestrators, `remaining_work.py`, the resync repair loop) are backend-agnostic and unchanged. No API key, MCP server, or network. Supports the full autonomous run loop — `list`/`get`/`set-status`/`comment` — with `set-status` editing only the ticket's `status:` line in place (comments and formatting preserved; reserialize fallback for irregular/JSON plans) and comments kept in a companion store so status writes never reflow the plan. Each ticket carries the canonical fields directly (tier/modules/resources/phase/depends_on/acceptance_criteria); `phase` is surfaced as a `phase:K` pseudo-label so phase-aware dispatch is identical to Linear. Grooming (`create`/`update`/`add-dependency`) is done by editing the YAML directly and the CLI points there. Default plan is gitignored (container-local, like the run log); point `local.plan` at a committed path to carry status across clones. YAML plans need PyYAML; `.json` plans need nothing (tests run with `uvx --with pytest --with pyyaml pytest`). Docs: `docs/local-tracker.md`.
